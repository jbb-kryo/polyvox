...
import {
  ...
  TrendRiderSettings,
  TrendRiderMetrics
} from '../../types/trendrider';
import {
  scanForMomentum,
  calculatePositionPnL,
  checkExitConditions
} from '../../services/momentumScanner';
import {
  checkUSDCBalance,
  estimateGasCost,
  calculateOrderCosts
} from '../../services/polymarketTrading';
import TrendOpportunitiesTable from './TrendOpportunitiesTable';
import TrendPositionsTable from './TrendPositionsTable';
import TrendTradeHistory from './TrendTradeHistory';
import TrendSettingsPanel from './TrendSettings';
import TrendMetrics from './TrendMetrics';

interface TrendRiderProps {
...
export default function TrendRider({ paperTradingMode, useCorsProxy, walletAddress, walletPrivateKey }: TrendRiderProps) {
  ...
  const [settings, setSettings] = useState<TrendRiderSettings>({
    minMomentumPercent: 5,
    positionSize: 5,
  ...
  useEffect(() => {
    const savedPositions = localStorage.getItem('trend_positions');
    const savedTrades = localStorage.getItem('trend_trades');
    const savedSettings = localStorage.getItem('trend_settings');
    const savedTodayStart = localStorage.getItem('trend_today_start');
    ...
    const lastReset = localStorage.getItem('trend_daily_reset');
    if (!lastReset || parseInt(lastReset) < midnight) {
      setTodayStartPnL(0);
      localStorage.setItem('trend_daily_reset', midnight.toString());
      localStorage.setItem('trend_today_start', '0');
    }
  }, []);

  useEffect(() => {
    localStorage.setItem('trend_positions', JSON.stringify(positions));
  }, [positions]);

  useEffect(() => {
    localStorage.setItem('trend_trades', JSON.stringify(trades));
  }, [trades]);
  ...
  }, [isScanning, settings.scanInterval, settings.minMomentumPercent, settings.momentumWindow, isDailyLimitReached]);
  ...
  const performScan = async () => {
    ...
    if (settings.cooldownEnabled && settings.lastLossTime) {
      const cooldownEnd = settings.lastLossTime + (settings.cooldownMinutes * 60 * 1000);
    ...
    setIsLoading(true);
    try {
      const opps = await scanForMomentum(
        settings.minMomentumPercent,
        settings.momentumWindow,
        useCorsProxy,
      ...
      if (opps.length > 0 && lastScanTime) {
        toast.success(`Found ${opps.length} momentum opportunities`);
      }

      if (settings.autoExecute && opps.length > 0 && positions.length < settings.maxConcurrentPositions) {
        handleExecuteTrade(opps[0]);
      }
    } catch (error) {
      console.error('Scan error:', error);
      toast.error('Failed to scan for momentum opportunities');
    } finally {
      setIsLoading(false);
    }
  };

  const handleAutoClose = (positionId: string, reason: 'profit_target' | 'stop_loss' | 'trailing_stop' | 'time_limit') => {
    const position = positions.find(p => p.id === positionId);
    if (!position) return;

    handleClosePosition(positionId, reason);
  };

  const handleExecuteTrade = async (opportunity: MomentumOpportunity) => {
  ...
  const calculatePositionSize = (): number => {
    if (settings.positionSizeMode === 'percent') {
      return (settings.totalCapital * settings.positionSizePercent) / 100;
    }
    return settings.positionSize;
  };

  const performSafetyChecks = async (): Promise<boolean> => {
    try {
      const balanceCheck = await checkUSDCBalance(walletAddress, walletPrivateKey);
      const requiredSize = calculatePositionSize();

      if (!balanceCheck.sufficient || balanceCheck.balance < requiredSize) {
        toast.error(`Insufficient balance. Have: $${balanceCheck.balance.toFixed(2)}, Need: $${requiredSize.toFixed(2)}`);
        return false;
      }
  ...
  const executeTrade = async (opportunity: MomentumOpportunity, isReal: boolean) => {
    const actualPositionSize = calculatePositionSize();

    if (isReal) {
      try {
        toast('Executing real trade...', { icon: '⚠️' });

        const direction = opportunity.direction === 'bullish' ? 'long' : 'short';

        const newPosition: TrendPosition = {
          id: `pos-real-${Date.now()}`,
          marketQuestion: opportunity.market.question,
          marketId: opportunity.market.id,
          direction,
          entryPrice: opportunity.market.currentPrice,
          currentPrice: opportunity.market.currentPrice,
          ...
          orderId: 'pending',
          txHashes: [],
          highestPrice: opportunity.market.currentPrice,
          lowestPrice: opportunity.market.currentPrice
        };
      ...
      setPositions(prev => [...prev, newPosition]);
      toast.success(`${direction.toUpperCase()} position opened ($${actualPositionSize.toFixed(2)})`);
      setShowConfirmModal(false);
    }
  };

  const handleClosePosition = (positionId: string, exitReason: 'profit_target' | 'stop_loss' | 'trailing_stop' | 'time_limit' | 'manual' = 'manual') => {
    const position = positions.find(p => p.id === positionId);
    if (!position) return;
    ...
    const newTrade: TrendTrade = {
      ...
      maxDrawdown: position.lowestPrice ? ((position.highestPrice! - position.lowestPrice) / position.highestPrice!) * 100 : 0
    };

    setTrades(prev => [...prev, newTrade]);
    setPositions(prev => prev.filter(p => p.id !== positionId));

    if (settings.cooldownEnabled && netProfit < 0 && (exitReason === 'stop_loss' || exitReason === 'trailing_stop')) {
      setSettings(prev => ({ ...prev, lastLossTime: Date.now() }));
      toast.error(`Loss recorded. Cooldown period started (${settings.cooldownMinutes} min)`);
    }

    const exitReasonText = exitReason.replace('_', ' ').toUpperCase();
    toast.success(`Position closed (${exitReasonText}): ${netProfit >= 0 ? '+' : ''}$${netProfit.toFixed(2)}`);
  };
  ...
  const resetDailyLimit = () => {
    ...
    localStorage.setItem('trend_today_start', todayStartPnL.toString());
    toast.success('Daily limit reset');
  };

  const metrics: TrendRiderMetrics = {
    ...
    profitTargetHits: trades.filter(t => t.exitReason === 'profit_target').length,
    stopLossHits: trades.filter(t => t.exitReason === 'stop_loss' || t.exitReason === 'trailing_stop').length,
  ...
  return (
    <div className="space-y-6">
      {!settings.realTradingMode ? (
        <div className="bg-yellow-900 bg-opacity-20 border border-yellow-700 rounded-lg p-4">
          ...
          <p className="text-yellow-200 text-sm mt-1">
            This bot simulates momentum trading. Enable Real Trading Mode in settings when ready.
          </p>
        </div>
      ) : (
      ...
      <TrendMetrics metrics={metrics} profitHistory={profitHistory} />

      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">Momentum Scanner</h2>
          {lastScanTime && (
      ...
      <div>
        ...
        {isLoading && opportunities.length === 0 ? (
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-8 text-center">
            ...
            <p className="text-gray-400 mt-4">Scanning for momentum...</p>
          </div>
        ) : (
          <TrendOpportunitiesTable
      ...
      <div>
        ...
        <TrendPositionsTable positions={positions} onClose={(id) => handleClosePosition(id, 'manual')} />
      </div>

      <div>
        <h3 className="text-xl font-bold text-white mb-4">Trade History ({trades.length})</h3>
        <TrendTradeHistory trades={trades} />
      </div>
            ...
            <div className="space-y-4 mb-6 text-white">
              <div className="bg-red-900 bg-opacity-30 border border-red-700 rounded-lg p-4">
                ...
                <ul className="list-disc list-inside space-y-1 text-sm text-red-200">
                  ...
                  <li>You can LOSE money - momentum trading is high risk</li>
                  <li>Gas fees apply to every transaction</li>
                  <li>Market prices can reverse suddenly</li>
                  <li>Stop losses may not execute perfectly</li>
              ...
              <div className="bg-gray-900 rounded-lg p-4">
                ...
                <ul className="list-disc list-inside space-y-1 text-sm text-gray-400">
                  ...
                  <li>Automatic profit targets and stop losses</li>
              ...
              <div className="text-sm text-gray-300">
                ...
                <ul className="list-disc list-inside space-y-1">
                  <li>You understand momentum trading risks</li>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg shadow-xl max-w-3xl w-full p-6 border border-red-600">
            ...
            <div className="space-y-4 mb-6">
              <div className="bg-gray-900 rounded-lg p-4">
                <h4 className="text-sm font-medium text-gray-400 mb-2">Market</h4>
                <p className="text-white text-sm">{selectedOpportunity.market.question}</p>
                <p className="text-gray-400 text-xs mt-1">
                  {selectedOpportunity.direction === 'bullish' ? 'Going LONG' : 'Going SHORT'}
                </p>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="bg-gray-900 rounded-lg p-3">
                  ...
                  <p className="text-lg font-bold text-white">${selectedOpportunity.market.currentPrice.toFixed(4)}</p>
                </div>
                <div className="bg-gray-900 rounded-lg p-3">
                  <p className="text-xs text-gray-400">Target</p>
                  <p className="text-lg font-bold text-green-400">+{settings.profitTargetPercent}%</p>
                </div>
                <div className="bg-gray-900 rounded-lg p-3">
                  <p className="text-xs text-gray-400">Stop</p>
                  <p className="text-lg font-bold text-red-400">-{settings.stopLossPercent}%</p>
                </div>
              </div>
          ...
          <div className="bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full p-6">
            ...
            <div className="space-y-4 mb-6">
              <div className="bg-gray-900 rounded-lg p-4">
                ...
                <p className="text-white text-sm">{selectedOpportunity.market.question}</p>
                <p className="text-gray-400 text-xs mt-1">
                  Direction: {selectedOpportunity.direction === 'bullish' ? 'LONG' : 'SHORT'} | Momentum: {selectedOpportunity.strength.toFixed(1)}%
                </p>
              </div>
