...
import { SnipeMasterSettings } from '../../types/snipemaster';

interface SnipeSettingsProps {
  settings: SnipeMasterSettings;
  onUpdate: (settings: SnipeMasterSettings) => void;
  onShowRealTradingWarning: () => void;
}

export default function SnipeSettings({ settings, onUpdate, onShowRealTradingWarning }: SnipeSettingsProps) {
      ...
      {isExpanded && (
        <div className="p-4 border-t border-gray-700 space-y-6">
          <div className={`p-4 rounded-lg border ${
            ...
            {settings.realTradingMode && (
              <div className="text-xs text-red-300 space-y-1">
                ...
                <p>Orders will be executed on Polygon blockchain with your wallet.</p>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Minimum Expected Profit: {settings.minProfitPercent}%
              </label>
              <input
                type="range"
                min="1"
                ...
                onChange={(e) => onUpdate({ ...settings, minProfitPercent: parseFloat(e.target.value) })}
                className="w-full"
              />
              <p className="text-xs text-gray-500 mt-1">
                Minimum profit target for placing orders
              </p>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => onUpdate({ ...settings, maxPositionSize: Math.max(10, parseFloat(e.target.value) || 100) })}
              ...
              <p className="text-xs text-gray-500 mt-1">
                Maximum amount per limit order
              </p>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Target Discount: {settings.targetDiscount}%
              </label>
              <input
                type="range"
                min="1"
                max="10"
                step="0.5"
                value={settings.targetDiscount}
                onChange={(e) => onUpdate({ ...settings, targetDiscount: parseFloat(e.target.value) })}
                className="w-full"
              />
              <p className="text-xs text-gray-500 mt-1">
                Place orders this % below current market price
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => onUpdate({ ...settings, timeoutMinutes: Math.max(5, parseInt(e.target.value) || 60) })}
                className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-blue-500"
              />
              <p className="text-xs text-gray-500 mt-1">
                Cancel orders after this time if not filled
              </p>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => onUpdate({ ...settings, dailyLossLimit: Math.max(5, parseFloat(e.target.value) || 50) })}
                  className="w-full pl-8 pr-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <p className="text-xs text-gray-500 mt-1">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => onUpdate({ ...settings, scanInterval: parseInt(e.target.value) as 15 | 30 | 60 })}
                className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-blue-500"
              >
          ...
          <div className="flex items-center justify-between p-4 bg-gray-900 rounded-lg">
            <div>
              <h4 className="text-sm font-medium text-gray-300">Auto-Execute Orders</h4>
              <p className="text-xs text-gray-500 mt-1">
                Automatically place limit orders when opportunities are found
              </p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={(e) => onUpdate({ ...settings, autoExecute: e.target.checked })}
                className="sr-only peer"
          ...
          {settings.autoExecute && !settings.realTradingMode && (
            <div className="bg-yellow-900 bg-opacity-20 border border-yellow-700 rounded-lg p-3">
              <p className="text-xs text-yellow-300">
                <strong>Warning:</strong> Auto-execute is enabled. Orders will be placed automatically without confirmation.
              </p>
            </div>
          )}
          ...
          {settings.realTradingMode && (
            <div className="bg-red-900 bg-opacity-20 border border-red-700 rounded-lg p-3">
              <p className="text-xs text-red-300">
                <strong>Note:</strong> Auto-execute is disabled in Real Trading Mode for safety. All orders require manual confirmation.
              </p>
            </div>
          )}
        </div>
